            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(spacing: 8) {
                        if viewModel.hasMoreHistory && !viewModel.isLoadingMore && initialScrollDone {
                            ProgressView()
                                .controlSize(.small)
                ...
                .onChange(of: viewModel.historyLoaded) {
                    if viewModel.historyLoaded && !initialScrollDone {
                        initialScrollDone = true
                        lastKnownCount = viewModel.messages.count
                    }
                }
                .onChange(of: viewModel.messages.count) {
                    let count = viewModel.messages.count
                    guard initialScrollDone else {
                        lastKnownCount = count
                        return
                    }
                    if let anchor = viewModel.scrollToAnchor {
                        viewModel.scrollToAnchor = nil
                        proxy.scrollTo(anchor, anchor: .top)
                        lastKnownCount = count
                        return
                    }
                    if count > lastKnownCount && !isAtBottom {
                        hasNewMessages = true
                    }
                    lastKnownCount = count
                }
