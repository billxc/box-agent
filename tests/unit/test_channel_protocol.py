...
class TestMockChannel:
    ...
    async def test_send_text_records(self):
        channel = MockChannel()
        msg_id = await channel.send_text("123", "hello")
        assert channel.sent_texts == [("123", "hello")]
    ...
    async def test_stream_lifecycle(self):
        ...
        await channel.stream_update(handle, "alpha")
        await channel.stream_update(handle, "alpha beta")
        msg_id = await channel.stream_end(handle)
    ...
    async def test_tool_call_recorded(self):
        channel = MockChannel()
        used = await channel.on_tool_call(
            "123", "tool-1", "Bash", {"cmd": "ls"}, "out",
        )
        assert used is False
        assert channel.tool_calls == [ToolCallRecord(
    ...
    async def test_tool_update_recorded(self):
        channel = MockChannel()
        used = await channel.on_tool_update(
            "123", "tc-7", "Run pwd", status="completed", output="/tmp",
        )
        assert used is False
        assert channel.tool_updates == [ToolUpdateRecord(
    ...
    async def test_tool_methods_can_signal_stream_break(self):
        channel = MockChannel(tool_call_uses_stream=True, tool_update_uses_stream=True)
        used_call = await channel.on_tool_call("c", "t", "n", {}, "r")
        used_update = await channel.on_tool_update("c", "t", "title")
    ...
    async def test_typing_recorded(self):
        channel = MockChannel()
        await channel.show_typing("123")
        await channel.show_typing("456")
        assert channel.typing_calls == ["123", "456"]
