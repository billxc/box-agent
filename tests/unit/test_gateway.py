...
class TestGateway:
    ...
    async def test_resumed_session_skips_context_injection(self, tmp_path):
        """When a saved session is restored, Router should NOT re-inject context."""
        from boxagent.gateway import Gateway
        ...
        gw = Gateway(config=mock_config, config_dir=tmp_path)
        gw._storage = MagicMock()
        gw._storage.load_session.return_value = "saved-session-123"
        gw._start_time = 1.0
        ...
        with patch("boxagent.gateway.ClaudeProcess") as MockCLI, \
            ...
            MockChan.return_value = AsyncMock()
            MockWD.return_value = MagicMock(run_forever=AsyncMock())

            await gw._start_bot("my-bot", bot_cfg)

        router = gw._routers["my-bot"]
        assert router._session_context_injected is True

    async def test_new_session_allows_context_injection(self, tmp_path):
        """When no saved session exists, Router should inject context on first message."""
        from boxagent.gateway import Gateway
        ...
        router = gw._routers["my-bot"]
        assert router._session_context_injected is False
