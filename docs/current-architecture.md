...
单个 Router ──────────────────────┐                  │
   ...
   WebChannel._publish ──► bus.publish("chat.<machine>.<bot>.<chat>")  （同一根 bus，chat.* 无 StoreSubscriber → 永不进库）
                                       ├─► 浏览器 SSE queue（bus.subscribe，local 或 remote 同一路径）
                                       └─► ChatSyncer（订 chat. 前缀的 bridge）──► PeerTransport ──► 跨机(host↔guest)
                                              ▲ 入站 chat_event 帧 ──► bus.publish 重注入本机 topic ─┘
...
- **一根 content-agnostic MessageBus（`bus/`）横切**：events(`events.*`) 和 chat(`chat.*`) 同一个实例，同步保序 fan-out。持久化/广播是 subscriber 行为（EventStore 只订 `events.`，所以 chat 永不进库）。**chat 的数据面本地/远端同 publish/subscribe**：`ChatSyncer` 是订 `chat.` 前缀的 bridge（转发给下游 peer）+ `watch_subscriptions` 传播 demand + 入站帧 `bus.publish` 重注入；`EventSyncer` 订 `events.` broadcast。两者共用 `PeerTransport` 发帧；出站 peer 帧走单条有序 drain（sync bus→async WS）。RPC 骑同一 transport（`rpc_over_bus`，request/reply，不进 publish）。event resync(cursor) 也是 query 不是 pub/sub —— **控制面各自骑 transport，数据面统一到 bus**。业务代码经 `boxagent.log` 写入，禁止直接 import `boxagent.events`
