...
- ✅ **跨机 shell 执行**：`POST /api/exec`（跨机走 `dispatch_machine_request` 中继，同步拿 `{exit_code, output}`）+ `scripts/boxrun <machine> "<cmd>"` CLI；与 Telegram `/exec` 命令共用 `shell_exec.run_shell_command`。**要目标机也跑新代码才有此端点。**
- ✅ **节点自检端点（不走 bus）**：`GET /api/version`（machine_id/version/commit）、`GET /api/status`（role / host 链路活值 / guest 列表 / uptime，本地直答、零 bus 依赖）——bus 坏时单机自证用。外部只有 host 经 devtunnel 可直连。
...
| `src/boxagent/shell_exec.py` | shell 执行核心 util（`run_shell_command`）——`/exec` 命令与 `POST /api/exec` 端点共用 || `src/boxagent/router/context.py` | 首条消息的 system prompt context 拼接 |
...
| `src/boxagent/transports/web/server.py` | Web UI HTTP server（含 `POST /api/exec` 跨机同步 shell 执行） || `src/boxagent/transports/web/log_file.py` | `read_tail()` — JSON-line 反向分页 + level/grep 过滤（Logs 页用） |
